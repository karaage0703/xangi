...
describe('Telegram group feedback', () => {
  ...
  it('acknowledges a queued album representative once after acceptance', async () => {
    const setMessageReaction = vi.fn().mockResolvedValue(true);
    const api = { setMessageReaction } as unknown as Pick<Api, 'setMessageReaction'>;
    const createTracker = vi.fn(() =>
      createTelegramReactionTracker(api, -100, 33, '👀', '🎉', vi.fn())
    );
    await withTelegramReactionOnAccepted(createTracker, async (onAccepted, reportOutcome) => {
    ...
    expect(setMessageReaction.mock.calls).toEqual([
      [-100, 33, [{ type: 'emoji', emoji: '👀' }]],
      [-100, 33, [{ type: 'emoji', emoji: '🎉' }]],
    ]);

    await withTelegramReactionOnAccepted(createTracker, async () => {});
    expect(createTracker).toHaveBeenCalledTimes(1);
  });
});
