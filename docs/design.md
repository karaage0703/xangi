...
- `autostart`が有効なextensionはxangi processの起動時に開始する。全体の`doctor`は各extensionの`doctor`も集約するが、1 extensionの起動失敗で他のchat platformまで停止させず、該当capabilityの利用時に明示エラーを返す
