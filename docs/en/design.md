...
- The Settings and Web Projects screens register existing absolute paths in the central workspace registry and can unregister unused workspaces. Unregistering never changes the directory or its files and is rejected for the default workspace, Project or existing-session references, and platform channel bindings
...
- `/schedules` is the schedule-management mode. Its HTTP APIs add, edit, enable, disable, delete, and manually run jobs for Web, Discord, Slack, and Telegram. Manual runs require confirmation, reuse the normal scheduler runner, and may run a paused job without changing its saved enabled state or next scheduled time. A second run of the same job is rejected while the first is active. Web jobs store a reserved new-conversation value as `channelId` plus an optional `projectId`. At run time the Project is validated again, a fresh Web session is created, and the normal Web agent-runner path appends the turn to that session
...
- Remote workers connect outbound to a dedicated WebSocket listener, separate from the internal Tool Server. A pre-registered worker authenticates with a file-backed token and advertises versioned capabilities. The MVP exposes system information, shell-free argv execution constrained by worker-local workspace and command allowlists, and read-only USB discovery. For diagnostics, `system.info` also reports the workspace roots and command allowlist currently loaded by the worker without exposing tokens, Gateway URLs, or configuration file paths. Device writes, serial control, automatic placement, and public-network transport are excluded until they have per-device approvals and TLS
- Remote worker lifecycle uses launchd on macOS and a systemd user service on Linux/WSL2. Pairing and private configuration persistence are shared; restart preserves credentials. On macOS, self-restart hands service replacement to a separate temporary launchd job so stopping the requesting worker cannot also terminate the process that registers its replacement. Linux checks the user manager before consuming a pairing code.
...
- Cache-aware compaction — only at a token threshold, replaces old complete turns with a structured checkpoint while retaining the recent tail in the same Session. It does not delete the leading message on every turn

**Context budget and cache-aware compaction (`runner.ts`: `loadContextBudget`):**

To align xangi's session limit with the LLM's `--max-model-len` (vLLM) or `num_ctx` (Ollama), limits are derived from env vars. History is not sliced or shifted on every turn. By default, one batch compaction runs when estimated history reaches 30% of `NUM_CTX`, keeping the prompt prefix stable between compaction boundaries so provider prefix caches remain reusable.
...
| `LOCAL_LLM_CONTEXT_KEEP_LAST`           | Message/character-trigger retention target (token budget takes priority) | `10` |
| `LOCAL_LLM_TOOL_RESULT_MAX_CHARS`       | Tool result truncation when inserted     | `4000`       |
| `LOCAL_LLM_MAX_SESSION_MESSAGES`        | Non-token fallback trigger               | `50`         |
| `LOCAL_LLM_COMPACTION_THRESHOLD_RATIO`  | Trigger ratio relative to `NUM_CTX`      | `0.30`       |
| `LOCAL_LLM_COMPACTION_KEEP_TOKENS`      | Recent-tail token budget (at most half the trigger)    | 10% of `NUM_CTX` (2000–12000) |
| `LOCAL_LLM_COMPACTION_COOLDOWN_MS`      | Retry delay after summary failure        | `60000`      |
| `LOCAL_LLM_IMAGE_ESTIMATE_TOKENS`       | Estimated tokens per image               | `2048`       |

Compaction retains complete recent user turns within the estimated token budget, including image estimates. Token limits take priority over `LOCAL_LLM_CONTEXT_KEEP_LAST`, which is only a retention target for message/character fallback triggers. The retained-token target is capped at half the compaction trigger to leave room for subsequent turns. The latest turn is always kept intact, even if it alone exceeds the target. A checkpoint without any newly removable history is not summarized again. Tool calls and results remain in the same turn. Failed summarization or checkpoint persistence preserves the original history and applies the existing retry cooldown.

The boundary is always a user message with a transcript ID, so assistant tool calls are never split from their tool results. Images use a fixed per-image token budget instead of base64 length. Old image base64 data is omitted from the summary request, while images in the recent tail remain intact. On success, xangi appends the summary, first retained message ID, and before/after token and message estimates to `DATA_DIR/logs/session-compactions/<appSessionId>.jsonl`. Restart restores the latest checkpoint plus normal transcript entries from that boundary. This side log is not exposed in Web Chat history. If summarisation or persistence fails, the original history remains unchanged and retries wait for the cooldown.
...
| cache-aware compaction                        | Estimated history reaches 30% of `NUM_CTX`, or the message/character fallback limit                                                                                                                                                                                                                                                      | Replace old complete turns with one structured checkpoint and keep the recent tail. Old tool results are reduced only at this boundary. Persist successful checkpoints in a separate JSONL; preserve original history and apply cooldown on failure                                                                                                                                                                                                                                                                          |
...
API: `recordToolCallAndDetectLoop(session, sig)` returns `{ kind: 'none' \| 'exact' \| 'similar', repeats? }` so `executeRunLoop` / `executeStreamLoop` can pick `repeatedToolCallErrorMessage` vs `similarToolCallErrorMessage` based on `kind`. `recordToolCallAndCheckLoop` is preserved as a boolean wrapper for backward compatibility. `planSessionCompaction` selects a complete-turn boundary, and `compactSessionWithCheckpoint` atomically replaces history only after both summarisation and persistence succeed. `compactOldToolResults` applies only to the old prefix sent for summarisation. `parsePseudoToolCall(text)` decomposes `call:fn{args}` into `{name, args}` via anchored grammar (returns `null` on failure). `isSafeForRescue(name, args)` returns `{safe, reason?}`.
...
- **cache-aware compaction**: checkpoint old complete turns only at a threshold and then keep the prompt prefix fixed → limits context growth without causing a cache miss on every turn.
...
### Extension favorites persistence

`ExtensionFavorites` atomically replaces `DATA_DIR/extension-favorites.json`, an ordered array of extension IDs. `GET /api/extension-favorites` resolves current catalog names and availability. Same-host guarded `POST` applies add/remove/up/down operations to the latest file instead of accepting a stale replacement array. Additions require an installed extension with a UI; missing entries remain removable. Corrupt files fail without being overwritten. Devices share host settings, refreshed on mount, focus, and local change events. Favorites do not alter runtime state or permissions.
