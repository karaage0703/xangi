...
複数枚をまとめて送ると、LINE は 1 枚ずつ別イベントで届ける。同じ `imageSet.id` のイベントを集め、`total` 枚そろった時点で 1 ターンとして処理する。返信は 1 回。

- 取得は届いた時点で始める。揃ってからまとめて取ると、最後の 1 枚が届いてから全部のダウンロードが走り、応答メッセージの期限を使い切る
- 添付は `index` 順に並べる。到着順は保証されない
- 締め切りは設けない。送信が完了するまで webhook は出ないため、揃わないのは送信中か送信失敗のどちらか
- 中断した時点で送信中だった 1 枚は、回線が戻ると継続されて同じ `imageSet.id` のまま遅れて届く。欠けがその 1 枚だけなら、待っていれば揃う
- **まだ送信が始まっていなかった分を送り直すと、`imageSet` が付かない。** LINE から見て別の送信になるため元のセットへは戻らず、待っていても揃わない
- 最後の到着から `LINE_SLOW_RESPONSE_THRESHOLD_MS` (default 45 秒) が過ぎても揃わないときは、待っていることを 1 回だけ伝える。本回答はしない
- 揃わないまま次の発言が来たら、その発言のターンへ画像を合流させる。送り直した分がセットへ戻らないため、この経路で拾う
- idle reset と `/new` (リセットコマンド) では、揃わなかった控えと取得済みのファイルを捨てる
