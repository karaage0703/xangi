...
`CURSOR_API_KEY` と `XAI_API_KEY` は通常のホワイトリストには含めず、それぞれ Cursor CLI / Grok CLI の子プロセスにだけ渡されます。
...
| 変数 | 説明 | デフォルト |
|------|------|-----------|
| `AGENT_BACKEND` | バックエンド（`claude-code` / `codex` / `cursor` / `grok` / `local-llm`） | `claude-code` |
| `AGENT_MODEL` | 使用するモデル | - |
...
| `XANGI_EVEN_TERMINAL_BACKEND` | Even Terminal 経由だけの backend default (`claude-code` / `codex` / `cursor` / `grok` / `local-llm`) | `AGENT_BACKEND` |
