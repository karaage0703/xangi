...
- [基本操作](#基本操作)
- [プラットフォーム別のメッセージ処理](#プラットフォーム別のメッセージ処理)
...
- 画像は到着時点から取得し、`index`順でLLMへ渡します
- 最後の到着から`LINE_SLOW_RESPONSE_THRESHOLD_MS`（デフォルト45秒）が過ぎてもそろわない場合、待機中であることを1回だけ通知します
- 未完成のまま次の発言が届いた場合、取得済み画像をそのターンへ合流させます
- 送信を中断して未送信分を送り直すと、LINE側で別送信として扱われ、元のセットへ戻らない場合があります
- idle resetまたは`/new`などのresetコマンドでは、未完成セットと取得済みファイルを破棄します
