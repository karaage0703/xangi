...
| `DISCORD_SESSION_TITLE_AI_ONCE` | `true`で、xangiが新規作成したDiscordスレッドの最初のturnだけAIタイトルを許可。既存スレッドや`/new`後の後続セッションは再命名しない | `false` |
...
`DISCORD_SESSION_TITLE_AI_ONCE=true`では、AIによる更新をxangiが新規作成したスレッドの最初のturnだけに制限します。既存スレッド、手動で付け直したスレッド名、`/new`後の新しい内部セッションは自動変更しません。
