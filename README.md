...
OSとCPUを自動判定してxangiを配置し、`~/.local/bin/xangi`を作成して、そのままAIとのセットアップとサービス起動まで進みます。どのディレクトリで実行しても構いません。pipeから起動してもAI選択はTerminalから入力できます。制御端末が無い場合、AIツールの導入・認証がまだの場合、またはAIセットアップが失敗した場合もxangi本体を保持して停止し、`xangi setup`から再開できます。`~/.local/bin`がPATHに無い場合は、installerが現在のshell用の`export PATH=...`とzshへの永続設定方法を表示します。
