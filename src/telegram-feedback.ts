...
export async function withTelegramReactionOnAccepted(
  createTracker: () => { finish(success: boolean): Promise<void> } | undefined,
  task: (onAccepted: () => void, reportOutcome: (success: boolean) => void) => Promise<void>
): Promise<void> {
  let reaction: ReturnType<typeof createTracker>;
  let success = false;
  let accepted = false;
  try {
    await task(
