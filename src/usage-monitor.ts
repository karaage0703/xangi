...
export interface AccountUsageProvider {
  id: 'codex' | 'antigravity' | 'github-copilot';
  label: string;
...
function runCodexAppServer(input: string, done: (stdout: string) => boolean): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => finish(new Error('Codex usage probe timed out')), TIMEOUT_MS);
    ...
    child.on('close', (code) => {
      if (!settled)
        finish(code === 0 ? undefined : new Error(stderr.trim() || `codex exited ${code}`));
    });
...
export async function readAccountUsage(
  ...
  readers: Array<() => Promise<AccountUsageProvider>> = [readCopilotUsage, readAntigravityUsage]
